...
async def transcribe_chunk(
    ...
    Full per-chunk pipeline:
      ...
      Step 6 — Dedup:        remove words repeated from previous chunk boundary
      Step 7 — Send:         send filtered transcript + tags to browser

    When voice lock is off (professor_embedding is None), skip steps 2-6 and send raw Whisper output.
    """
    try:
        ...
        # Step 4: WeSpeaker — compare each segment vs professor embedding
        ...
        transcript, _ = stitch_professor_words(words, professor_segments)
        if not transcript:
            logger.debug("[chunk] no words remained after stitch")
            return

        # Step 6: Dedup — remove words repeated at the 2s chunk overlap boundary
...
def get_vad_regions(samples: np.ndarray) -> list[tuple[float, float]]:
    """
    Slide VAD across the full chunk, return (start_sec, end_sec) speech regions.

    Why do we need this in the live pipeline?
    Segmentation model needs continuous speech to work properly.
    VAD first removes silence gaps — without it, the segmentation model
    would try to find speaker boundaries in silence, producing garbage segments.

    Regions are padded by VAD_PAD_SEC and merged to avoid cutting words at boundaries.
    """
    h  = np.zeros((2, 1, 64), dtype=np.float32)
    c  = np.zeros((2, 1, 64), dtype=np.float32)
    sr = np.array(SAMPLE_RATE, dtype=np.int64)
    ...
    for i in range(0, len(samples) - VAD_WINDOW_SIZE + 1, VAD_WINDOW_SIZE):
        ...
        scores.append((i / SAMPLE_RATE, float(outs[0].squeeze())))
        h, c = outs[1], outs[2]
...
def stitch_professor_words(
    ...
    Why midpoint? A word spans start→end. Using midpoint avoids edge cases
    where a word starts just before a segment boundary but is clearly inside it.
    """
    kept = []
    for w in words:
        mid = (w['start'] + w['end']) / 2.0
        for (seg_start, seg_end) in professor_segments:
            if seg_start <= mid <= seg_end + 0.5:
                kept.append(w['word'])
                break
